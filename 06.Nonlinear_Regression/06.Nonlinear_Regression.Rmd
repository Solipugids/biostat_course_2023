...
$$
对观测值$y$分解为三部分$\alpha$，$\beta x$, $\epsilon$

1. 这分别代表什么？
2. 如何进行方差分解，以及判断回归模型的显著性？
3. 回归分析和相关分析之间的差别是什么？
...
knitr::include_graphics("The-Michaelis-Menten-Equation-in-biochemistry.png")
```
...
knitr::include_graphics("./poorr2.png")
```
Good $R^2$
```{r echo=FALSE, fig.align='center', out.height="30%"}
knitr::include_graphics("./goodr2.png")
```

## 第一节$~$非线性回归的直线化 $~$`一、曲线类型的确定`

均方根误差 Root Mean Square Error $RMSE = \sqrt\frac{\sum(\hat{y} – y)²}{N}$ 


Low
```{r echo=FALSE, fig.align='center', out.height="30%"}
knitr::include_graphics("./rmse_high.png")
```
High
```{r echo=FALSE, fig.align='center', out.height="30%"}
knitr::include_graphics("./rmse_low.png")
```
...
knitr::include_graphics("./Lineweaver-Burke_plot.png")
```

Drawing a straight line is usually more accurate than drawing a curve line.
...
```
  - 之后该模型失去了预测能力，有可能反映了哪些美国历史上的重大事件，如内战、大萧条、两次世界大战？
