...
plot(x, y, type = "l", xlim = c(-4, 4), ylim = c(0, 0.7), main = expression(paste(N(mu == 0, sigma^2 == 1), ", ", alpha, "=0.05", ", one-tailed")), axes = FALSE, ylab = "Probability")
...
样本平均数$\bar{x}$所属总体平均数$\mu$与已知总体平均数$\mu_0$是否有差异- 总体方差$\sigma^2$已知
...
## 第三节$~$样本频率的假设检验 $~$`二、一个样本频率的假设检验`

## 第三节$~$样本频率的假设检验 $~$`二、两个样本频率的假设检验`
...
$P(\bar{x} - 1.96 \sigma_{\bar{x}} \le \bar{x} \le \bar{x} + 1.96 \sigma_{\bar{x}}) = 0.95$
$P(\bar{x} - 2.58 \sigma_{\bar{x}} \le \bar{x} \le \bar{x} + 2.58 \sigma_{\bar{x}}) = 0.99$

- 置信区间 $[L_1 = \bar{x} - u_\alpha \sigma_{\bar{x}}, L_2 = \bar{x} + u_\alpha \sigma_{\bar{x}}]$
...
## 第四节$~$参数的区间估计与点估计 $~$`二、一个总体平均数的区间估计与点估计`

## 第四节$~$参数的区间估计与点估计 $~$`三、两个总体平均数差数的区间估计与点估计`
