...
knitr::include_graphics("./eraser.jpg")
...
- 极值定理：如果函数$f(x)$是在区间$[a,b]$上连续的函数，必然存在最大值和最小值，并且取到最大值$max$和最小值$min$至少一次
  - 通过几何面积关系看极值定理
...
- 研究$y$和$\hat{y}$可以得到如下回归方程的三个基本性质：
  ...
  1. 最小值 $minQ=\sum (y-\hat{y})^2$
  2. $\sum(y - \hat{y}) = 0$
  3. 回归直线通过中心点$(\bar{x}, \bar{y})$
...
```{r echo=FALSE, crop = TRUE, fig.align='center', fig.asp=0.5}
par(mfrow= c(1,2))
...
- $F$检验：通过F分布比较回归平方和与残差平方和，以确定他们之间是否存在线性关系
...
$$
\rho = \frac{1}{N}\sum[(\frac{x - \mu_x}{\sigma_x})(\frac{y - \mu_y}{\sigma_y})]  = \frac{\sum(x-\mu_x)(y-\mu_y)}{\sqrt{\sum (x-\mu_x)^2 \times \sum (y-\mu_y)^2}}
$$
