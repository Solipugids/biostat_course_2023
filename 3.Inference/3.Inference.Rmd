...
knitr::include_graphics("./msphere.jpg")
```

> Akkermansia muciniphila and Lactobacillus plantarum ameliorate systemic lupus erythematosus by possibly regulating immune response and remodeling gut microbiota. 2023. mSphere.

## 第一节$~$假设检验的原理与方法 $~$`一、假设检验的概念`

- $p$值（$p$-value，$p$ val）

  - 生物医学数据中用到统计学分析，最直观的变量就是$p$值
  - $p$值就是概率（probability）值，反映某一事件发生的可能性大小
  - 除了$p$值，还有$q$，FDR值等统计检验的变量
